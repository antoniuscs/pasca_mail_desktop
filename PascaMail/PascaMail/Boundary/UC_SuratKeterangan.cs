...
{
    public partial class UC_SuratKeterangan : UserControl
    {
        public UC_SuratKeterangan()
        {
            InitializeComponent();
        }
        ...
        private void UC_SuratKeterangan_Load(object sender, EventArgs e)
        {
            cleartxt();
        ...
        private bool cektxt()
        {
            ...
            if (dateTesis.Text == dateWisuda.Text)
            {
                errorProvider1.SetError(dateTesis, "Silahkan pilih tanggal Tesis...");
                errorProvider1.SetError(dateWisuda, "Silahkan pilih tanggal Wisuda...");
                dateTesis.Focus();
                temp = false;
            }
            if (dateSurat.Text == dateWisuda.Text)
            {
                errorProvider1.SetError(dateSurat, "Silahkan pilih tanggal Surat...");
                dateTesis.Focus();
                temp = false;
            }
            if (dateSurat.Text == dateTesis.Text)
            {
                errorProvider1.SetError(dateSurat, "Silahkan pilih tanggal Surat...");
                dateSurat.Focus();
                temp = false;
        ...
        private void btnSelesai_Click(object sender, EventArgs e)
        {
            ...
            if (dr == DialogResult.Yes)
            {
                if (cektxt() == true)
                {
                    ...
                    PascaMail.Entity.Surat S = new Entity.Surat(idsurat, lblHalSurat.Text, lblNPM.Text, dateSurat.Text, "", "", dateWisuda.Text, dateTesis.Text, IDMakul, "", IDStaff, IDStaff, IDStatus);
                    MC.inputSurat(S);
